...
package sg.gaiaframework.assets 
{
	...
	/**
	 * @author	etc
	 * Original class: ru.etcs.utils.FontLoader
	 * 
	 * repackaged //with(out) permission//, by Glenn Ko
	 */
	public class FontLoaderAsset extends ByteArrayAsset
	{
		
		public function FontLoaderAsset() 
		{
			super();
		}
		...
		private static function init():void {
			if (FontLoaderAsset._initialized) return;
			...
			ba.position = 0;
			ba.readBytes(FontLoaderAsset.SWF_HEADER);
			ba.length = 0;
			ba.writeBytesFromString(
			...
			ba.position = 0;
			ba.readBytes(FontLoaderAsset.CLASS_CODE);
			ba.length = 0;
			FontLoaderAsset._initialized = true;
		}
		...
		override public function parseNode(page:IPageAsset):void {
			...
			_autoRegister = _node.@autoRegister == "false" ? false : true;
		}
		
		
		
		override public function init():void {
			FontLoaderAsset.init();
			super.init();
		}
		
		
		override protected function onComplete(event:Event):void {
			this.analyze(loader.data as ByteArray);
		...
		private function handler_libComplete(event:Event):void {
			...
			for (i = 0;i < this._fontCount;i++) {
				...
				id = FontLoaderAsset.CLASS_NAME_PREFIX + id;
		...
		private function analyze(bytes:ByteArray, delayed:Boolean = false):void {
			...
			var classCodeLength:uint = FontLoaderAsset.CLASS_CODE.length;
			...
			tempData = new ByteArray();
			tempData.endian = Endian.LITTLE_ENDIAN;
			tempData.writeBytes(FontLoaderAsset.SWF_HEADER);
			id = 0;
			...
			for (o in fontData) {
				...
				if (data) {
					...
					stringID = FontLoaderAsset.CLASS_NAME_PREFIX + stringID;
					tempData.writeShort(FontLoaderAsset.TAG_DO_ABC);
					...
					tempData.writeBytes(FontLoaderAsset.CLASS_CODE);
					tempData.writeBytes(data);
					tempData.writeShort(FontLoaderAsset.TAG_SYMBOL_CLASS);
